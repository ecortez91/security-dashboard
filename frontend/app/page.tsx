                                  ...
                                  {fix.manualSteps && fix.manualSteps.length > 0 && (
                                    <div className="mt-2 sm:mt-3 p-2 sm:p-3 bg-slate-950 rounded-lg border border-slate-700">
                                      <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                                        <p className="text-slate-300 text-[10px] sm:text-xs font-semibold">📋 Manual Steps:</p>
                                        <button
                                          onClick={() => {
                                            const commands = fix.manualSteps
                                              .filter((step: string) => step.startsWith('Run:'))
                                              .map((step: string) => step.replace('Run: ', '').trim());
...
`;
                                            ...
                                            const a = document.createElement('a');
                                            a.href = url;
                                            a.download = `${fix.id || 'fix'}-script.sh`;
                                            a.click();
                                            URL.revokeObjectURL(url);
                                          }}
                                          className="px-2 py-1 bg-slate-700 hover:bg-slate-600 text-white text-[10px] sm:text-xs font-medium rounded transition-all"
                                        >
                                          Download script ⬇️
                                        </button>
                                      </div>
